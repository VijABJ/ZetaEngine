...
var
  bSuccess: boolean;
begin
  ZbLib_AppFrame_Init (hInstance, WindowClassName, WindowTitle, iWidth, iHeight);
  Result := S_OK;
  {//
  Result := S_FALSE;
...
begin
  Result := S_FALSE;
  if (CoreEngine <> NIL) then begin
    CoreEngine.SetBackgroundMusic (string (lpszMusicName));
    Result := S_OK;
  end;
end;
...
begin
  Result := S_FALSE;
  if (CoreEngine <> NIL) then begin
    CoreEngine.PlaySound (string (lpszSoundName));
    Result := S_OK;
  end;
end;
...
begin
  g_CmdQueue.Insert (EventCommand, 0, 0);
