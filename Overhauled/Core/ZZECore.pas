...
begin
  if (AControl <> NIL) then begin
    AControl.Hide;
    g_CmdQueue.Insert (cmRemoveDialog, 0, Integer (AControl));
    //EventQueue.InsertEvent (cmRemoveDialog, 0, integer (AControl));
  end;
end;
...
var
  Desktop: TZEDesktop;
begin
  TraceLn ('::TZEGameCore.PostInitialize - ENTER');
  Music := TRUE;
  //
  ...
  {if (ScriptMaster.CheckHandler (PChar (SCRIPT_GAME_GUI))) then begin
    TraceLn ('Calling Client UI Generator');
    ScriptMaster.CallHandler (PChar (SCRIPT_GAME_GUI), ScreenWidth, ScreenHeight);
  end;}
  //
  ...
  //TraceLn ('MouseDev = NIL? = [%s]', [IfThen (MouseDev = NIL, 'TRUE', 'FALSE')]);
  //TraceLn ('MouseDev.SetVisible (TRUE);');
    ...
    case Event.m_Command of
      ...
      cmRemoveDialog: begin
        if (Event.m_pData <> NIL) then
          ...
          //
        g_DXFramework.ClearEvent (Event);
        g_CmdQueue.Insert (cmDialogClosed, 0, 0);
      end;
    end;
  end;
  //
