...
begin
  with FMap do begin
    ...
    MapId := Self.Name;
    GridSprite := CoreEngine.SpriteFactory.CreateSprite (SPRITE_FAMILY_SELECTORS, SPRITE_MAP_GRID);
    SelectionSprite := CoreEngine.SpriteFactory.CreateSprite (SPRITE_FAMILY_SELECTORS, SPRITE_MAP_SELECTOR);
    HighlightSprite := CoreEngine.SpriteFactory.CreateSprite (SPRITE_FAMILY_SELECTORS, SPRITE_MAP_HIGHLIGHT);
    TransitionSprite := CoreEngine.SpriteFactory.CreateSprite (SPRITE_FAMILY_SELECTORS, SPRITE_MAP_TRANSITION);
    StartPointSprite := CoreEngine.SpriteFactory.CreateSprite (SPRITE_FAMILY_SELECTORS, SPRITE_MAP_STARTPOINT);
  end;
end;
...
begin
  ...
  g_CmdQueue.InsertWithStr (cmDeletedArea, 0, PChar (FActiveArea.Name));
...
begin
  ...
  if (FActiveArea <> NIL) then
    g_CmdQueue.InsertWithStr (cmUnloadingArea, 0, PChar (FActiveArea.Name));
    //EventQueue.InsertEvent (cmUnloadingArea, PChar (FActiveArea.Name));
  //
  FActiveArea := theArea;
  //
  if (FActiveArea <> NIL) then
    g_CmdQueue.InsertWithStr (cmLoadingArea, 0, PChar (FActiveArea.Name));
...
begin
  ...
  g_CmdQueue.Insert (cmWorldLoaded, 0, 0);
end;
