...
type
  ...
  protected
    ...
    procedure SetScrGrid (AScrGrid: IZESprite);
  public
    ...
    property ScrGrid: IZESprite read FScrGrid write SetScrGrid;
  ...
  private
    ...
    FGridSprite: IZESprite;
    FHighlightSprite: IZESprite;
    FSelectionSprite: IZESprite;
    FMarkerTransitionSprite: IZESprite;
    FMarkerStartSprite: IZESprite;
    FTileSelected: TZEViewTile;
    //
  ...
  public
    ...
    //
    property GridSprite: IZESprite read FGridSprite write SetGridSprite;
    property HighlightSprite: IZESprite read FHighlightSprite write SetHighlightSprite;
    property SelectionSprite: IZESprite read FSelectionSprite write FSelectionSprite;
    property TransitionSprite: IZESprite read FMarkerTransitionSprite write FMarkerTransitionSprite;
    property StartPointSprite: IZESprite read FMarkerStartSprite write FMarkerStartSprite;
...
//////////////////////////////////////////////////////////////////////////
procedure TZEViewTile.SetScrGrid (AScrGrid: IZESprite);
begin
  FScrGrid := NIL;
  FScrGrid := AScrGrid;
end;
  ...
  if (Surface.TerrainSprite <> NIL) then with Surface do begin
    ...
    // otherwise, draw the terrain
      ...
      for iIndex := 0 to Pred (Surface.TransitionsCount) do begin
        transSprite := Surface.Transitions [iIndex];
        if (transSprite = NIL) then continue;
        transSprite.Position := FDrawRef;
        transSprite.Draw (TRUE);
      end;
    end;
  end;
  // draw the grid if necessary
  if (ScrGrid <> NIL) AND (GlobalViewShowGrid) then begin
    ScrGrid.Position := FDrawRef;
    ScrGrid.Draw (TRUE);
  end;
  // draw transitions if necessary 
  if (GlobalViewShowPortals) AND (Portal <> NIL) then begin
    if (Portal.Kind = ptTransition) then
      transSprite := TZEViewMap (Owner.Owner).FMarkerTransitionSprite
      else transSprite := TZEViewMap (Owner.Owner).FMarkerStartSprite;
    //
    if (transSprite <> NIL) then begin
      transSprite.Position := FDrawRef;
      transSprite.Draw (TRUE);
    end;
  end;
...
begin
  for X := 0 to Pred (Width) do
    for Y := 0 to Pred (Height) do begin
      theTile := TZEViewTile (Self [X, Y]);
      if (theTile <> NIL) then theTile.ScrGrid := AGridSprite;
    end;
  //
end;
...
begin
  ...
  FMarkerTransitionSprite := NIL;
  FMarkerStartSprite := NIL;
  FTileSelected := NIL;
  //
