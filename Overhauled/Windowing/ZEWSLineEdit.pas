  ...
  if (((Event.m_Event AND evKeyboard) <> 0) AND GetState (stFocused)) then
    begin
      case Event.m_Key of
        ...
        #8:             // BACKSPACE
          if ((FCurLen > 0) AND (FCurPos > 0)) then
            begin
...
begin
  ...
  FOldConsoleMode := g_Keyboard.InConsoleMode;
  g_Keyboard.SetConsoleMode (TRUE);
  //
...
begin
  g_Keyboard.SetConsoleMode (FOldConsoleMode);
  //DI8SetConsoleMode (FOldConsoleMode);
  inherited EndModal;
end;
...
begin
  ...
  // otherwise, ignore it.
  if (NOT GetState (stFocused)) then
    begin
      bPointInside := ContainsScreenPoint (Event.m_Pos);
...
begin
  if (CEntry = '') then
    ...
    Result := (StrToInt (CEntry) <= FMaximum);
end;
...
var
  Ed: TZEEdit;
begin
  Ed := TZEEdit.Create (rBounds);
  if (Ed <> NIL) then
    begin
      if (AFont <> NIL) then
        Ed.Font := AFont
      else
        Ed.Font := GUIManager.Fonts ['StandardEdit'];
      //
    end;
  //
  Result := Ed;
...
var
  Ed: TZENumericEdit;
begin
  Ed := TZENumericEdit.Create (rBounds);
  if (Ed <> NIL) then
    begin
      if (AFont <> NIL) then
        Ed.Font := AFont
