...
type
  ...
  public
    ...
    //
    function GenerateLOS (StartTile, TargetTile: TZETile): TList; overload;
    function GenerateLOS (StartPos, TargetPos: TPoint): TList; overload;
    function GenerateLOS (StartPos: TPoint; TargetTile: TZETile): TList; overload;
    function GenerateLOS (StartTile: TZETile; TargetPos: TPoint): TList; overload;
    //
...
//////////////////////////////////////////////////////////////////////////
function TZELevel.GenerateLOS (StartTile, TargetTile: TZETile): TList;
var
  P0, P1, PT, Delta: TPoint;

  procedure AddPoint (P: TPoint);
  var
    theTile: TZETile;
  begin
    //TraceLn ('Adding: P (%d, %d)', [P.X, P.Y]);
    theTile := Self [P.X, P.Y];
    if (theTile <> NIL) then Result.Add (theTile);
  end;

  procedure Octant0 (P, D: TPoint; XIncr: Integer);
  var
  ...
  procedure Octant1 (P, D: TPoint; XIncr: Integer);
  var
    DXx2, DXx2MinusDYx2, ErrorTerm: Integer;
  begin
    DXx2 := Delta.X * 2;
    DXx2MinusDYx2 := DXx2 - (Delta.Y * 2);
    ErrorTerm := DXx2 - Delta.Y;
    //
    AddPoint (P);
    Dec (Delta.Y);
    while (Delta.Y > 0) do begin
      if (ErrorTerm >= 0) then begin
        Inc (P.X, XIncr);
        Inc (ErrorTerm, DXx2MinusDYx2);
      end else begin
        Inc (ErrorTerm, DXx2);
      end;
      //
...
begin
  ...
  if (StartTile = NIL) OR (TargetTile = NIL) then Exit;
  // get points...
  P0 := StartTile.FGridPosition;
  P1 := TargetTile.FGridPosition;
  // swap if P1 is a point lower on the map
  ...
  //
  //TraceLn ('P0 (%d, %d), P1 (%d, %d)', [P0.X, P0.Y, P1.X, P1.Y]);
  Result := TList.Create;
  // calculate  length
  Delta := Point (P1.X - P0.X, P1.Y - P0.Y);
  if (Delta.X > 0) then begin
    if (Delta.X > Delta.Y) then
      Octant0 (P0, Delta, 1)
      else Octant1 (P0, Delta, 1);
    //
  end else begin
    Delta.X := -Delta.X;
    if (Delta.X > Delta.Y) then
...
//////////////////////////////////////////////////////////////////////////
function TZELevel.GenerateLOS (StartPos, TargetPos: TPoint): TList;
begin
  Result := GenerateLOS (Self [StartPos.X, StartPos.Y], Self [TargetPos.X, TargetPos.Y]);
end;

//////////////////////////////////////////////////////////////////////////
function TZELevel.GenerateLOS (StartPos: TPoint; TargetTile: TZETile): TList;
begin
  Result := GenerateLOS (Self [StartPos.X, StartPos.Y], TargetTile);
end;

//////////////////////////////////////////////////////////////////////////
function TZELevel.GenerateLOS (StartTile: TZETile; TargetPos: TPoint): TList;
begin
  Result := GenerateLOS (StartTile, Self [TargetPos.X, TargetPos.Y]);
end;
