...
type
  ...
  public
    ...
    //
    procedure TeleportTo (X, Y, Z: integer); overload;
    procedure TeleportTo (ToTile: TZEViewTile); overload;
    procedure Displace (ADirection: TZbDirection);
    procedure SwapWith (Target: TZEEntity);
    //
    function DirectionTo (Target: TZEEntity): TZbdirection;
  ...
  public
    ...
    //
    procedure ReplacePC (cMasterName: String;
      cWorkingName: String = '$$PC$$'; Handler: TZERemoteEntityCallback = NIL);
    procedure CreatePC (cMasterName: String;
      cWorkingName: String = '$$PC$$'; Handler: TZERemoteEntityCallback = NIL);
    procedure ClearPC;
              ...
              if (AQ_Count = 0) then begin
                ...
                if (AnchorTile.Portal <> NIL) AND
                   (AnchorTile.Portal.Kind = ptTransition) then begin
                    //TraceLn ('TRANSITION ENCOUNTERED');
...
begin
  ...
  case Event of
    ...
    //
    eeDoActionOther: begin
      AtTile := TZEViewTile (pParam1);
      if (AtTile <> NIL) then pParam2 := AtTile.UserData;
      Result := CallHandler (Event, pParam1, pParam2, lParam1, lParam2);
    end;
    //
    else begin
...
begin
  ...
  //
  if (AnchorTile.IsNeighbor (Target.AnchorTile)) then begin
    Result := TRUE;
    Exit;
  end;
  //
  LOS := AnchorTile.Owner.GenerateLOS (AnchorTile, Target.AnchorTile);
  if (LOS = NIL) then Exit;
  //
  //TraceLn ('CanSee(), checking LOS Tiles...');
  for Index := 1 to (LOS.Count-2) do begin
    theTile := TZEViewTile (LOS [Index]);
    //Trace ('(%d, %d)...', [theTile.GridX, theTile.GridY]);
    if (theTile.Surface.Terrain = NIL) OR (theTile.UserData <> NIL) then Exit;
    //TraceLn ('OK');
  end;
  //
  LOS.Free;
...
begin
  ...
  //
  if (FSpan.X > 1) OR (FSpan.Y > 1) then begin
    //
    theLevel := Target.AnchorTile.Owner;
    anchorX := Target.AnchorTile.GridX;
    anchorY := Target.AnchorTile.GridY;
    for X := 0 to Pred (FSpan.X) do begin
      for Y := 0 to Pred (FSpan.Y) do begin
        // skip anchor tile, it's already processed
        if (X = 0) AND (Y = 0) then continue;
        //
        theTile := TZEViewTile (theLevel [anchorX+X, anchorY + Y]);
        OtherDistance := AnchorTile.HowFarTo (theTile);
        if (OtherDistance < Result) then Result := OtherDistance;
      end;
    end;
  end;
end;
...
//////////////////////////////////////////////////////////////////////////
procedure TZEEntity.TeleportTo (X, Y, Z: integer);
var
  theLevel: TZELevel;
  theTile: TZETile;
begin
  ...
  if (theLevel = NIL) then Exit;
  theTile := theLevel [X, Y];
  TeleportTo (TZEViewTile (theTile));
end;

//////////////////////////////////////////////////////////////////////////
procedure TZEEntity.TeleportTo (ToTile: TZEViewTile);
begin
  if (ToTile = NIL) OR (ToTile.UserData <> NIL) then Exit;
  //
  Unplace;
  PlaceAt (ToTile, FALSE);
  AQ_Clear;
end;

//////////////////////////////////////////////////////////////////////////
procedure TZEEntity.Displace (ADirection: TZbDirection);
var
  vtTarget: TZEViewTile;
begin
  if (NOT OnMap) OR (ADirection = tdUnknown) then Exit;
  vtTarget := TZEViewTile (AnchorTile.Neighbors [ADirection]);
  TeleportTo (vtTarget);
end;

//////////////////////////////////////////////////////////////////////////
procedure TZEEntity.SwapWith (Target: TZEEntity);
var
  targetTile: TZEViewTile;
  ourTile: TZEViewTile;
begin
  // be sure both entities are in the map, and target is valid
  if (NOT OnMap) OR (Target = NIL) OR (NOT Target.OnMap) then Exit;
  // catch this one, just in case
  if (Target = Self) then Exit;
  // objects being swapped must be the same size
  if (Target.FSpan.X <> FSpan.X) OR (Target.FSpan.Y <> FSpan.Y) then Exit;
  // remember the locations
  targetTile := Target.AnchorTile;
  ourTile := AnchorTile;
  // uproot both entities
  Target.Unplace;
  Unplace;
  // replace them, changing places here...
  Target.PlaceAt (ourTile);
  PlaceAt (targetTile);
end;

//////////////////////////////////////////////////////////////////////////
function TZEEntity.DirectionTo (Target: TZEEntity): TZbdirection;
begin
  if (Target = NIL) OR (Target.GameAreaOwner <> GameAreaOwner) then
    Result := tdUnknown
    else Result := AnchorTile.GetDirectionTo (Target.AnchorTile);
end;
...
begin
  ...
  if (Event = eeTriggerPortal) then begin
    ...
    //TraceLn ('Transition Request, Target={%s}', [cAreaName]);
    if (theArea = NIL) then Exit;
    //
