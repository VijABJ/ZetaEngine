...
var
  ...
  g_ConfigManager: TZbConfigManager = NIL;
  //
  g_LastTick: Cardinal = 0;
  g_ElapsedTicks: Cardinal = 0;
  g_MusicVolume: longint = 100;
  //
...
begin
  ...
  FBounds := EmptyRect;
  //
  g_ConfigManager := TZbConfigManager.Create;// (AConfigFile);
...
begin
  ...
  //
  TraceLn ('LoadImageVolume (%s)', [cVolumeFile]);
...
begin
  ...
  TraceLn ('--- Initializing DirectMusic/DirectSound');
  ZbLib_GetAudioManager (g_AudioManager);
  g_AudioManager.Init(hWindow, TRUE, SND_MNGR_DEF_FREQUENCY, 32);
  // Initialize DirectShow.
  TraceLn ('--- Initializing DirectShow');
  g_SoundFXMngr := TZESoundFXManager.Create;
...
begin
  if (Music) then g_MusicMngr.Play (AMusicName, g_MusicVolume);
end;
...
////////////////////////////////////////////////////////////////////
procedure TZEFrameworkDX.ResetMusicVolumes;
begin
  g_MusicMngr.ResetNewVolume (g_MusicVolume);
end;

////////////////////////////////////////////////////////////////////
procedure TZEFrameworkDX.PlaySound (ASoundName: PChar);
begin
  if (SoundFX) then g_SoundFXMngr.Play (ASoundName);
end;
...
////////////////////////////////////////////////////////////////////
function AppFrameDelegateProc (WWEvent: TZbWinWrapEvent; AParam: Pointer;
  AWinHandle: HWND; winMsg: PZbWinMsg): LongBool; stdcall;
...
begin
  if (g_AppWrap <> NIL) then
    g_AppWrap.AddDelegate (AppFrameDelegateProc, NIL, 'ZEDXFrameWork', TRUE);
end;
