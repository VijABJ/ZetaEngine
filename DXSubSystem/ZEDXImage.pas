...
type
  ...
  public
    constructor Create (ZbImage: TZbBitmap32; ATransparent: boolean = TRUE); overload;
    constructor Create (AFilename: string; ATransparent: boolean = TRUE); overload;
    constructor Create (Source: TZEImage); overload;
  ...
  protected
    ...
    //
    procedure __UnloadRef (AName: String);
...
begin
  FZbImage.Free;
  FDXSurface := NIL;
  if (FListOwner <> NIL) then FListOwner.__UnloadRef (Name);
  inherited;
end;
...
begin
  ...
  if (Image <> NIL) then begin
    Image.Name := AName;
    Image.FListOwner := Self;
    Result := Image;
  end;
end;

//////////////////////////////////////////////////////////////////////////
procedure TZEImageList.__UnloadRef (AName: String);
var
  Descriptor: PZE_ImageDescriptor;
begin
  Descriptor := PZE_ImageDescriptor (FImages.Get (AName));
  if (Descriptor = NIL) then Exit;
  Descriptor.pImage := NIL;
end;
...
var
  IList: IZbEnumStrings;
  cParam1, cParam2: string;
  bParam1, bParam2: boolean;
  bIsAlias: boolean;
  cData: string;
begin
  IList := g_ConfigManager.LoadSimpleConfig (AConfigFile);
