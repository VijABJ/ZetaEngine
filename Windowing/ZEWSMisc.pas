...
var
  Gap, Filled, W, H: integer;
begin
  if (FRange <= 0) then exit;
  //
  Filled := FCurrent - FMin;
  FFilledRect := LocalBounds;
  with FFilledRect do begin
    W := Right - Left;
    H := Bottom - Top;
    //
    if (FStyle = gsHorizontal) then begin
      Gap := Round (W * (Filled / FRange));
      if (FDirection = gdLeftToRight) then
        Right := Gap
        else Left := W - Gap;
      //
    end else begin (* gsVertical *)
      Gap := Round (H * (Filled / FRange));
...
begin
  ...
  if (ShowCaption) then begin
    Caption := IntToStr (Current) + '/' + IntToStr (Max);
    DrawCaption;
  end;
end;

///////////////////////////////////////////////////////////////////
procedure TZEProgressGauge.ChangeBounds (rBounds: TRect);
begin
  inherited;
  IValueChanged;
end;
...
///////////////////////////////////////////////////////////////////
function TZEProgressGauge.GetPropertyValue (APropertyName: string): string;
begin
  if (APropertyName = PROP_NAME_FILLER_COLOR) then
    Result := ColorToString (FFillColor)
  else
    Result := inherited GetPropertyValue (APropertyName);
  //
end;
...
///////////////////////////////////////////////////////////////////
function TZEProgressGauge.SetPropertyValue (APropertyName, Value: string): boolean;
begin
  Result := TRUE;
  //
  if (APropertyName = PROP_NAME_FILLER_COLOR) then
    FillColor := StrToColorRef (Value)
  else
    Result := inherited SetPropertyValue (APropertyName, Value);
  //
end;
...
///////////////////////////////////////////////////////////////////
procedure TZETextBox.CheckScrollbarUse;
var
  bUseScrollBar: LongBool;
begin
  bUseScrollBar := (FTextView.FItems.Count > 0) AND
    (FTextView.FItems.Count <= FTextView.FLinesVisible);
  //
  if (bUseScrollbar) then
    FScrollbar.Show
    else FScrollbar.Hide;
end;
