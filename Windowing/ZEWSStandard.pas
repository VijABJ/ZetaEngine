...
type
  ...
  private
    ...
    FAnimationNotify: Boolean;
    FConsumeClick: Boolean;
    FNextSprite: PChar;
    //
  ...
  protected
    ...
    procedure SetNextSprite (cSprite: String);
    function GetNextSprite: String;
    //
    procedure RefreshBoundsFromSprite;
    ...
    property AnimationNotify: Boolean read FAnimationNotify write FAnimationNotify;
    property ConsumeClick: boolean read FConsumeClick write FConsumeClick;
  public
  ...
  public
    ...
    property AutoWidth;
    property AutoHeight;
    property AnimationNotify;
    property ConsumeClick;
  end;
...
begin
  ...
  ChangeMouseEventMask ([evLBtnClick]);
  //
  FDrawPattern := [psRepeatX, psRepeatY];
  FAnimates := false;
  FAnimationTick := 0;
  FAnimationTimer := NIL;
  FAnimationNotify := TRUE;
  FConsumeClick := FALSE;
  FNextSprite := NIL;
end;
...
begin
  ...
  if (FAnimationTimer = NIL) then begin
...
//////////////////////////////////////////////////////////
procedure TZECustomDecorImage.MouseLeftClick (var Event: TZbEvent);
var
  theEvent: TZbEvent;
begin
  if (NOT FConsumeClick) then Exit;
  MakeCommandEvent (theEvent, cmDecorClicked, Integer (Self));
...
begin
  if (FAnimates <> bAnimates) then begin
    FAnimates := bAnimates;
...
begin
  if (FAnimationTick <> dwAnimationTick) then begin
...
begin
  if (IsRepeatX <> bRepeatX) then begin
    if (bRepeatX) then
      FDrawPattern := FDrawPattern + [psRepeatX]
      else FDrawPattern := FDrawPattern - [psRepeatX];
  end;
end;
...
begin
  if (IsRepeatY <> bRepeatY) then begin
    if (bRepeatY) then
      FDrawPattern := FDrawPattern + [psRepeatY]
      else FDrawPattern := FDrawPattern - [psRepeatY];
  end;
end;
...
begin
  if (IsCenterX <> bCenterX) then begin
    if (bCenterX) then
      FDrawPattern := FDrawPattern + [psCenterX]
      else FDrawPattern := FDrawPattern - [psCenterX];
  end;
end;
...
begin
  if (IsCenterY <> bCenterY) then begin
    if (bCenterY) then
      FDrawPattern := FDrawPattern + [psCenterY]
      else FDrawPattern := FDrawPattern - [psCenterY];
  end;
end;
...
var
  R: TRect;
begin
  if (HasAutoWidth <> bAutoWidth) then begin
    if (bAutoWidth) then begin
...
var
  R: TRect;
begin
  if (HasAutoHeight <> bAutoHeight) then begin
    if (bAutoHeight) then begin
...
//////////////////////////////////////////////////////////
procedure TZECustomDecorImage.SetNextSprite (cSprite: String);
begin
  if (FNextSprite <> NIL) then begin
    StrDispose (FNextSprite);
    FNextSprite := NIL;
  end;
  //
  if (cSprite <> '') then FNextSprite := StrNew (PChar (cSprite));
end;

//////////////////////////////////////////////////////////
function TZECustomDecorImage.GetNextSprite: String;
begin
  Result := IfThen (FNextSprite = NIL, '', String (FNextSprite));
end;
...
begin
  ...
  //
  R := LocalBounds;
  if (NOT HasAutoWidth) then begin
    if (IsCenterX) then begin
      R.Left := R.Left + ((Width - DefaultSprite.Width) div 2);
      R.Right := R.Left + DefaultSprite.Width;
    end else if (NOT IsRepeatX) then
      R.Right := R.Left + Min (Width, DefaultSprite.Width);
    //
  end;
  //
  if (NOT HasAutoHeight) then begin
    if (IsCenterY) then begin
      R.Top := R.Top + ((Height - DefaultSprite.Height) div 2);
      R.Bottom := R.Top + DefaultSprite.Height;
    end else if (NOT IsRepeatY) then
...
//////////////////////////////////////////////////////////
procedure TZECustomDecorImage.RefreshBoundsFromSprite;
var
  R: TRect;
begin
  R := Bounds;
...
begin
  if (DefaultSprite = NIL) then Exit;
  //
  if (DefaultSprite.AtLastFrame) then begin
    DefaultSprite.FirstFrame;
    if (FAnimationNotify) then begin
      MakeCommandEvent (Event, cmDecorEndSequence, Integer (Self));
      InsertEvent (Event);
      //
      if (FNextSprite <> NIL) then begin
        SpriteName := GetNextSprite;
        SetNextSprite ( '');
      end;
    end;
  end else
...
begin
  ...
  else if (APropertyName = PROP_NAME_CONSUME_CLICK) then
    Result := BooleanToProp (FConsumeClick)
  else
...
begin
  ...
  if (APropertyName = PROP_NAME_REPEAT_X) then
    ...
    ConsumeClick := PropToBoolean (Value)
  else if (APropertyName = PROP_NAME_NEXT_SPRITE) then
    SetNextSprite (Value)
  else
