...
uses
  ...
  var
    __ScrollLeft: Boolean = FALSE;
    __ScrollRight: Boolean = FALSE;
    __ScrollUp: Boolean = FALSE;
    __ScrollDown: Boolean = FALSE;
  ...
  //////////////////////////////////////////////////////////////////////////
  function ScrollHandler (AKey: Integer; AEvent: TZbKeyState;
    AUserData: Integer): LongBool; stdcall;
  begin
    Result := TRUE;
    //
    case AKey of
      DIK_LEFT:   __ScrollRight := (AEvent = ksPressed);
      DIK_RIGHT:  __ScrollLeft := (AEvent = ksPressed);
      DIK_UP:     __ScrollDown := (AEvent = ksPressed);
      DIK_DOWN:   __ScrollUp := (AEvent = ksPressed);
    end;
    //
    with TZEGameWindow (AUserData) do begin
      if (FViewMap = NIL) OR (KeyboardScroll) then Exit;
      //
      if (__ScrollLeft) then FViewMap.PanVirtualView (tdWest, KEYBOARD_SCROLL_SPEED);
      if (__ScrollRight) then FViewMap.PanVirtualView (tdEast, KEYBOARD_SCROLL_SPEED);
      if (__ScrollUp) then FViewMap.PanVirtualView (tdNorth, KEYBOARD_SCROLL_SPEED);
      if (__ScrollDown) then FViewMap.PanVirtualView (tdSouth, KEYBOARD_SCROLL_SPEED);
    end;
    //
  end;
...
begin
  ...
  //
  g_EventManager.Keyboard.AddKeyHook (ScrollHandler, DIK_LEFT, Integer (Self));
  g_EventManager.Keyboard.AddKeyHook (ScrollHandler, DIK_RIGHT, Integer (Self));
  g_EventManager.Keyboard.AddKeyHook (ScrollHandler, DIK_UP, Integer (Self));
  g_EventManager.Keyboard.AddKeyHook (ScrollHandler, DIK_DOWN, Integer (Self));
end;
...
begin
  ...
  //
  // handle the keyboard, if it's there!
  ...
  //
  // handle mouse moves here, or mouse at edges situations..
  if (Event.m_Event = evMouseMove) then begin
    ...
    if (AtTile <> FTileAtCursor) then begin
      ...
      //if (FTileAtCursor <> NIL) then begin
        ClearEvent (E, TRUE);
        MakeCommandEvent (E, cmNewTileHighlighted, 0);
        //E.m_Event := evCommand;
