...
type
  ...
  public
    ...
    procedure ShowPromptDialog (iWidth, iHeight: Integer;
      cPrompt: String; iCommand: Integer);
    //
...
var
  cImageVolume: string;
  SList: IZbEnumStrings;
begin
  with g_ConfigManager do begin
    SList := ReadConfigSection (REZ_DATA_SECTION);
    try
      cImageVolume := SList.First;
      while (cImageVolume <> '') do begin
        TraceLn ('Loading: %s', [cImageVolume]);
...
begin
  ...
  TerrainManager := TZETerrainManager.Create (
    g_ConfigManager.ReadConfigSection (REZ_TERRAIN_SECTION,
    sroNamesAndValues)) as IZETerrainManager;
  WallManager := TZEWallManager.Create (
    g_ConfigManager.ReadConfigSection (REZ_WALLS_SECTION,
...
begin
  ...
  if (Event.m_Event <> evDONE) then begin
    //TraceLn ('Sending Command: %d', [Event.m_Command]);
...
begin
  ...
  try
    ...
    SpriteFactory.LoadSprList (CreateEnumStrings (StrList));
  finally
    StrList.Free;
  end;
end;
...
begin
  ...
  // alternate code, run dialog from root window!
  //FWinSysRoot.Insert (Dialog);
...
////////////////////////////////////////////////////////////////////
procedure TZEGameCore.ShowPromptDialog (iWidth, iHeight: Integer;
  cPrompt: String; iCommand: Integer);
var
  PD: TZEPromptDialog;
begin
  PD := TZEPromptDialog.Create (Rect (0, 0, iWidth, iHeight));
  if (PD = NIL) then Exit;
  //
  PD.SetMessage (cPrompt);
  PD.CommandToGenerate := iCommand;
  RunDialog (PD);
end;
