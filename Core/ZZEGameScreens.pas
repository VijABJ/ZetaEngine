...
begin
  ...
  with CSI^ do begin
    if (PSpriteName <> NIL) then begin
      OldSprite := DefaultSprite;
      SpriteName := string (PSpriteName);
      //
      if (OldSprite <> NIL) then begin
        OldImage := TZEImage (OldSprite.__SpecialGetImage);
        OldImage.Free;
      end;
    end;
    //
...
begin
  ...
  if (FCurrentScene >= Pred (Scenes.Count)) then begin
    FCurrentScene := 0;
    Inc (FLoopCount);
    g_EventManager.Commands.Insert (cmCutSceneHasLooped, 0, 0);
...
begin
  g_EventManager.Commands.Insert (cmCutSceneBegins, 0, 0);
