...
var
  bSuccess: boolean;
begin
  ZbLib_AppFrame_Init (hInstance, FALSE, WindowClassName, WindowTitle, TRUE, iWidth, iHeight);
  Result := S_OK;
  {//
  Result := S_FALSE;
...
//////////////////////////////////////////////////////////////////////////
function TZEUIManager.GetXPos (Control: IZEUIControl): Integer; stdcall;
begin
  try
    Result := TZEControl (Control).Left;
  except
    Result := 0;
  end;
end;
...
//////////////////////////////////////////////////////////////////////////
function TZEUIManager.GetYPos (Control: IZEUIControl): Integer; stdcall;
begin
  try
    Result := TZEControl (Control).Top;
  except
    Result := 0;
  end;
end;
...
//////////////////////////////////////////////////////////////////////////
function TZEUIManager.GetWidth (Control: IZEUIControl): Integer; stdcall;
begin
  try
    Result := TZEControl (Control).Width;
  except
    Result := 0;
  end;
end;
...
//////////////////////////////////////////////////////////////////////////
function TZEUIManager.GetHeight (Control: IZEUIControl): Integer; stdcall;
begin
  try
    Result := TZEControl (Control).Height;
  except
    Result := 0;
  end;
end;
...
//////////////////////////////////////////////////////////////////////////
procedure TZEUIManager.MoveTo (Control: IZEUIControl; NewX, NewY: Integer); stdcall;
var
  rBounds: TRect;
  Delta: TPoint;
begin
  try
    rBounds := TZEControl (Control).Bounds;
    ...
    TZEControl (Control).Bounds := rBounds;
  except
  end;
end;

//////////////////////////////////////////////////////////////////////////
procedure TZEUIManager.MoveRel (Control: IZEUIControl; DeltaX, DeltaY: Integer); stdcall;
var
  rBounds: TRect;
  Delta: TPoint;
begin
...
//////////////////////////////////////////////////////////////////////////
procedure TZEUIManager.Resize (Control: IZEUIControl; NewWidth, NewHeight: Integer); stdcall;
var
  rBounds: TRect;
  Delta: TPoint;
begin
...
//////////////////////////////////////////////////////////////////////////
procedure TZEUIManager.Delete (Control: IZEUIControl); stdcall;
begin
  try
    TZEControl (Control).Free;
  except
  end;
end;
...
//////////////////////////////////////////////////////////////////////////
procedure TZECore.ShowMsgBoxEx (lpfnMessage: PChar; Left, Top, Right, Bottom, Command: integer); stdcall;
begin
  if (CoreEngine <> NIL) then
    CoreEngine.ShowMsgBox (String (lpfnMessage), Rect (Left, Top, Right, Bottom), Command);
end;
...
//////////////////////////////////////////////////////////////////////////
procedure TZECore.ShowPromptDialog (iWidth, iHeight: Integer;
  lpszPrompt: PChar; iCommandToGenerate: Integer); stdcall;
begin
...
//////////////////////////////////////////////////////////////////////////
procedure TZECore.AddKeyHook (KeyCode: Integer; UserHandler: TZbKeyCallback); stdcall;
begin
  g_EventManager.Keyboard.AddKeyHook (UserHandler, KeyCode, 0);
end;

//////////////////////////////////////////////////////////////////////////
procedure TZECore.ClearKeyHook (KeyCode: Integer); stdcall;
begin
  g_EventManager.Keyboard.AddKeyHook (NIL, KeyCode, 0);
end;

//////////////////////////////////////////////////////////////////////////
procedure TZECore.ToggleHighlight (bActive: Integer); stdcall;
begin
  g_VisibleHighlight := (bActive <> 0);
end;

//////////////////////////////////////////////////////////////////////////
function TZECore.RandomInt (Range: Integer): Integer; stdcall;
begin
  Result := Random (Range);
end;

//////////////////////////////////////////////////////////////////////////
procedure TZECore.SetMusicVolume (AVolumePercent: Integer); stdcall;
begin
  if (AVolumePercent < 0) then AVolumePercent := 0;
  if (AVolumePercent > 100) then AVolumePercent := 100;
  g_MusicVolume := AVolumePercent;
  CoreEngine.ResetMusicVolumes;
end;
...
//////////////////////////////////////////////////////////////////////////
procedure TZEZetaEntity.Displace (Direction: TZbDirection); stdcall;
begin
  while (Direction = tdUnknown) do Direction := GetRandomDirection;
  FEntity.Displace (Direction);
end;

//////////////////////////////////////////////////////////////////////////
procedure TZEZetaEntity.Teleport (DestRef: Pointer); stdcall;
begin
  FEntity.TeleportTo (TZEViewTile (DestRef));
end;

//////////////////////////////////////////////////////////////////////////
procedure TZEZetaEntity.SwapWith (Target: IZEZetaEntity); stdcall;
begin
  FEntity.SwapWith (TZEEntity (Target.GetInternalData));
end;

//////////////////////////////////////////////////////////////////////////
function TZEZetaEntity.DirectionTo (Target: IZEZetaEntity): TZbDirection; stdcall;
begin
  if (Target <> NIL) then
    Result := FEntity.DirectionTo (TZEEntity (Target.GetInternalData))
    else Result := tdUnknown;
end;

//////////////////////////////////////////////////////////////////////////
function TZEZetaEntity.GetLocationX: Integer; stdcall;
begin
  try
    Result := FEntity.AnchorTile.GridX;
  except
    Result := 0;
  end;
end;
...
//////////////////////////////////////////////////////////////////////////
function TZEZetaEntity.GetLocationY: Integer; stdcall;
begin
  try
    Result := FEntity.AnchorTile.GridY;
  except
    Result := 0;
  end;
end;
...
//////////////////////////////////////////////////////////////////////////
function TZEZetaEntity.GetLocationZ: Integer; stdcall;
begin
  Result := 0;
end;

//////////////////////////////////////////////////////////////////////////
procedure TZEZetaEntity.TeleportTo (X, Y, Z: Integer); stdcall;
begin
  try
    FEntity.TeleportTo (X, Y, Z);
  except
  end;
end;
...
var
  cAreaName: String;
begin
  if (lpszAreaName = NIL) then
    cAreaName := GameWorld.ActiveArea.Name
    else cAreaName := String (lpszAreaName);
  //
  GameWorld.DropPC (cAreaName, X, Y, Z);
end;
...
begin
  Result := 0;
  //TraceLn('DropEntity() ENTER');
  //TraceLn ('DropEntity(%s)', [String (lpszEntityBase)]);
...
//////////////////////////////////////////////////////////////////////////
function TZEZetaWorld.GetCurrentAreaName: Integer; stdcall;
var
  S: String;
  IString: IZEString;
begin
  if (GameWorld.ActiveArea <> NIL) then
    S := GameWorld.ActiveArea.Name
    else S := '';
  //
  IString := TZEString.Create (S) as IZEString;
  IString._AddRef;
  Result := Integer (IString);
end;

//////////////////////////////////////////////////////////////////////////
function TZEZetaWorld.Save (lpszWorldFile: PChar): HRESULT; stdcall;
begin
  GameWorld.SaveToFile (String (lpszWorldFile));
  Result := S_OK;
end;

//////////////////////////////////////////////////////////////////////////
function TZEZetaWorld.GetEntity3 (X, Y, Z: Integer): Integer; stdcall;
var
  tile: TZEViewTile;
begin
  Result := 0;
  with GameWorld do begin
    if (Z < 0) OR (Z >= ActiveArea.Map.Count) then Exit;
    if (X < 0) OR (X >= ActiveArea.Map.Width) then Exit;
    if (Y < 0) OR (Y >= ActiveArea.Map.Height) then Exit;
    //
    tile := TZEViewTile (ActiveArea.Map [Z][X, Y]);
    if (tile = NIL) then Exit;
    //
    Result := Integer (tile.UserData);
  end;
end;

//////////////////////////////////////////////////////////////////////////
function TZEZetaWorld.CheckLocation (X, Y, Z: Integer): Integer; stdcall;
begin
  Result := Ord (FALSE);
  with GameWorld do begin
    if (Z < 0) OR (Z >= ActiveArea.Map.Count) then Exit;
